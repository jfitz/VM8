...
def format_generated_bytes(opcode, arg_size, arg_value, output_base)
  s = format_byte(opcode, output_base)
  
  s += ' '
  
  # print LSB of arg
  if arg_size > 0
    s += format_byte(arg_value % 256, output_base)
  else
    s += format_byte(nil, output_base)
  end
...
def format_output(offset, opcode, arg_size, arg_value, label, mnemonic, arg_text, output_base)
  s = ''
  ...
  s += format_generated_bytes(opcode, arg_size, arg_value, output_base)
...
def format_nongen_output(label, comment, offset, output_base)
  s = ''
  
  if label.nil?
    # no label means no offset, just spaces (and only then if a comment)
    if comment.size > 0
      s += format_word(nil, output_base) + '  '
      s += (format_byte(nil, output_base) + ' ') * 3
      s += '#'
    end
  else
...
def format_bytes_output(offset, values, output_base)
  s = ''
  ...
  values.each do |value|
    s += format_byte(value, output_base)
    s += ' '
  end
...
while line = gets
  ...
  unless asm_text.empty?
    if asm_text[0] == '.'
      ...
      if opcode.nil?
        list_line = format_nongen_output(label, comment, offset, output_base)
      else
        ...
        # add to code segment
        list_line = format_output(offset, opcode, arg_size, arg_value, label, mnemonic, arg_tokens, output_base)
...
symbols.each do |symbol, abs_rel_value|
  value_s = format_word(abs_rel_value.value, output_base)
  abs_or_rel = "abs"
