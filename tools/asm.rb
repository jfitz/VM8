...
def format_output(offset, opcode, arg_size, arg_value, label, mnemonic, arg_text, verbose)
  s = ''

  # print offset
  s += format_octal_word(offset) + ': ' if verbose
...
def format_nongen_output(label, comment, offset, verbose)
  s = ''
...
def format_bytes_output(offset, values, verbose)
  s = ''
  ...
  unless asm_text.empty?
    if asm_text[0] == '.'
      ...
      when '.label'
        ...
        # store value in labels table (check for inconsistency)
      when '.equate'
      ...
      when '.word'
        ...
        print format_bytes_output(offset, bytes, verbose)
        if value.is_rel
          if parts.size == 1
            label = parts[0]
          else
      ...
      if mnemonic.nil?
        print format_nongen_output(label, comment, offset, verbose)
      else
        ...
        # add to code segment
        print format_output(offset, opcode, arg_size, arg_value, label, mnemonic, arg_text, verbose)
